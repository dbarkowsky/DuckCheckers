...
<div class="game-card">
	...
	<StaticBoard {game} />
  {#if !game.players[PlayerPosition.TWO]}<button class="black-button" on:click={() => handleJoinClick('black')}>Join as Black</button>
	{:else}<span class="player-name black">{game.players[PlayerPosition.TWO].playerName}</span>{/if}
	<span class="vs-span">VS</span>
  {#if !game.players[PlayerPosition.ONE]}<button class="red-button" on:click={() => handleJoinClick('red')}>Join as Red</button>
